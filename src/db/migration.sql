...
CREATE INDEX IF NOT EXISTS idx_memories_category ON memories(user_id, category) WHERE is_active = TRUE;
...
CREATE INDEX IF NOT EXISTS idx_summaries_user ON conversation_summaries(user_id, period_end DESC);
...
CREATE INDEX IF NOT EXISTS idx_scheduled_pending ON scheduled_messages(scheduled_for) WHERE sent_at IS NULL;
...
CREATE INDEX IF NOT EXISTS idx_proactive_daily ON proactive_message_log(user_id, date);
...
CREATE TRIGGER tr_memories_updated BEFORE UPDATE ON memories FOR EACH ROW EXECUTE FUNCTION update_updated_at();
